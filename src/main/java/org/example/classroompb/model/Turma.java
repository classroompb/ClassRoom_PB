...
public class Turma implements Serializable {
    ...
    public List<String> getAlunoMatriculados() {
        return new ArrayList<>(alunoMatriculados);
    }

    public int getTotalMatriculados() {
        return alunoMatriculados.size();
    }

    public List<String> getAlunosEmEspera() {
        return new ArrayList<>(alunosEmEspera);
    } // RF21

    public int getTotalEmEspera() {
        return alunosEmEspera.size();
    } // RF21
    ...
    /**
     * Ajusta o limite de vagas mantendo a coerência com as vagas disponíveis. Ao aumentar o limite,
     * novas vagas são liberadas (base para o RF24). Não permite reduzir abaixo do número de alunos
     * já matriculados.
     */
    public void ajustarLimiteVagas(int novoLimite) {
        ...
        if (novoLimite < matriculados) {
            throw new IllegalArgumentException(
                    "Novo limite ("
                            + novoLimite
                            + ") não pode ser menor que o número de alunos já matriculados ("
                            + matriculados
                            + ").");
        }
