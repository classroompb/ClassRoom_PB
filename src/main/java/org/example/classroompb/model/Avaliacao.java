...
/**
 * Avaliação de um aluno em uma turma (Release 3 / RF31+).
 * Guarda as notas lançadas, a frequência e a situação final.
 *
 * Regras aplicáveis: RN07 (nota 0..10), RN08 (frequência mínima 75%),
 * RN09/RN10/RN11 (faixas de média) e RN12 (falta prevalece sobre a nota).
 * O cálculo da média e da situação final é feito no serviço (task D2).
 */
public class Avaliacao implements Serializable {
    ...
    public SituacaoFinal getSituacao() { return situacao; }
    ...
    /** Fecha a avaliação: a partir daqui as notas não podem mais ser editadas (RF de edição — task L2). */
    public void fechar() {
        this.fechada = true;
    }

    /**
     * Lança uma nota validando RN07 (0 a 10). Bloqueia se a avaliação já
     * estiver fechada — base para a task L2 (edição antes do fechamento).
     */
    public void adicionarNota(double nota) {
        if (fechada) {
            throw new IllegalArgumentException("Avaliação fechada — não é possível lançar/editar notas.");
        }
        if (nota < 0 || nota > 10) {
