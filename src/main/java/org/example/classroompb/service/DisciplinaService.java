...
public class DisciplinaService {
	...
	public Disciplina cadastrar(String codigo, String nome, int cargaHora, int creditos) {
		...
		if (codigo == null || codigo.isBlank()) throw new IllegalArgumentException("Código de disciplina inválido");
		if (nome == null || nome.isBlank()) throw new IllegalArgumentException("Nome da disciplina inválido");

		if (jaExiste(codigo)) throw new IllegalArgumentException("Já existe uma disciplina com o código - " + codigo);
		if (cargaHora <= 0) throw new IllegalArgumentException("Carga horária deve ser maior que zero");
	...
	public void adicionarPreRequisito (String codigoDisciplina, String codigoPreReq) {
		...
		if (disciplina == null) throw new IllegalArgumentException("Disciplina não encontrada - " + codigoDisciplina);
		
		if (buscarPorCodigo(codigoPreReq) == null) throw new IllegalArgumentException("Pré-requisito não encontrado - " + codigoPreReq);
