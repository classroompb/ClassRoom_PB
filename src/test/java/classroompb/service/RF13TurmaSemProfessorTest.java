...
class RF13TurmaSemProfessorTest {
	...
	static class TurmaRepositorioFake extends TurmaRepository {
		...
		@Override
		public List<Turma> carregarTodos() { return new java.util.ArrayList<>(lista); }
		@Override
		public void salvarTodos(List<Turma> turmas) { lista.clear(); lista.addAll(turmas); }
	}
	static class UsuarioRepositorioFake extends UsuarioRepository {
		private final List<Usuario> lista = new java.util.ArrayList<>();
		@Override
		public List<Usuario> carregarTodos() { return new java.util.ArrayList<>(lista); }
		@Override
		public void salvarTodos(List<Usuario> usuarios) { lista.clear(); lista.addAll(usuarios); }
	}
	static class DisciplinaRepositorioFake extends DisciplinaRepository {
		...
		@Override
		public List<Disciplina> carregarTodos() { return new java.util.ArrayList<>(lista); }
		@Override
		public void salvarTodos(List<Disciplina> disciplinas) { lista.clear(); lista.addAll(disciplinas); }
	}
	static class PeriodoLetivoRepositorioFake extends PeriodoLetivoRepository {
		...
		@Override
		public List<PeriodoLetivo> carregarTodos() { return new java.util.ArrayList<>(lista); }
		@Override
		public void salvarTodos(List<PeriodoLetivo> periodos) { lista.clear(); lista.addAll(periodos); }
	}
	...
	void deveImpedirOfertaTurmaComMatriculaProfessorVazia() {
		...
		assertTrue(ex.getMessage().toLowerCase().contains("professor") || ex.getMessage().toLowerCase().contains("vazia"));
	}
	...
	void deveImpedirOfertaTurmaComMatriculaProfessorNula() {
		PeriodoLetivo periodo = periodoLetivoService.buscarPorIdentificador("2024.1");
		Exception ex = assertThrows(IllegalArgumentException.class, () ->
			turmaService.ofertarTurma("CC101", null, periodo, 30, "08:00-10:00", "Sala A1")
		);
		assertTrue(ex.getMessage().toLowerCase().contains("professor") || ex.getMessage().toLowerCase().contains("nulo"));
	}
}
